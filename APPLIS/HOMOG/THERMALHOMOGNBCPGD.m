classdef THERMALHOMOGNBCPGD
    % THERMALHOMOGNBCPGD class
    ...
    methods
        function obj=THERMALHOMOGNBCPGD(size_domain,nb_elem,ls,neg_side,pos_side,delta,epsi,varargin)
        ...
        function obj=solve_mode(obj,i,varargin)
            ...
            [obj.u_free{i},obj.result{i}] = solve(A,obj.BC{i},solver);
            obj.u_unfree{i}=obj.u_free{i};
            for j=1:obj.dim
                obj.u_unfree{i}=freevector(obj.u_unfree{i},j,obj.model{j});
            end
        end
        ...
        function obj=findeffectiveconductivity(obj,varargin)
            ...
            for i=1:obj.dim
                for j=1:obj.dim
                    Au=multisvd(expand(obj.A_unfree*obj.u_unfree{j}),varargin{:});
                    obj.C(i,j)=prodscal(obj.u_unfree{i},Au);
                end
            end
            obj.C=obj.C/volume;
            obj.C=inv(obj.C);
        ...
        function []=export(obj,filename)
            ...
            if obj.dim==2
                ...
                for d=1:obj.dim
                    ...
                    fwrite(fid, ['<DataArray type="Float32" Name="Mode' num2str(d) '" format="ascii" NumberOfComponents="1">' nl]);
                    for j=1:N
                        for i=1:M
                ...
                for d=1:obj.dim
                    uu=expand(obj.q{d});
                    fwrite(fid, ['<DataArray type="Float32" Name="Flux' num2str(d) '" format="ascii" NumberOfComponents="3">' nl]);
                    for j=1:(N-1)
                        for i=1:(M-1)
    ...
    methods (Access = private)
        ...
        function obj=definebcs(obj)
            ...
            if obj.dim==2
                ...
                obj.BC{1} = SEPMATRIX({xm,obj.lin{2};xp,obj.lin{2}});
                obj.BC{2} = SEPMATRIX({obj.lin{1},ym;obj.lin{1},yp});
            elseif obj.dim==3
                xm=zeros(obj.nb_elem(1)+1,1);
                xm(1)=-1;
                xp=zeros(obj.nb_elem(1)+1,1);
                ...
                obj.BC{1} = SEPMATRIX({xm,obj.lin{2},obj.lin{3};xp,obj.lin{2},obj.lin{3}});
                obj.BC{2} = SEPMATRIX({obj.lin{1},ym,obj.lin{3};obj.lin{1},yp,obj.lin{3}});
                obj.BC{3} = SEPMATRIX({obj.lin{1},obj.lin{2},zm;obj.lin{1},obj.lin{2},zp});
            end
        end
        ...
        function obj=buildoperators(obj)
            if obj.dim==2
                ...
                obj.A_unfree = obj.A_unfree + SEPMATRIX({Lx*Lx',Ly*Ly'},obj.epsi);
            ...
            elseif obj.dim==3
                ...
                Ly=obj.lin{2};
                Lz=obj.lin{3};
                obj.A_unfree = obj.A_unfree + SEPMATRIX({Lx*Lx',Ly*Ly',Lz*Lz'},obj.epsi);
            end
        end
    end
end
