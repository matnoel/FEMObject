...
[Pinter,repP,numnode]=intersect(POINT(P),POINT(getnode(S)));
numnode = getnumber(getnode(S),numnode);
...
if ~isenrich(elem)
    [vtemp,repP] = eval_sol(elem,getnode(S),ud,P(repP2(:)),ddl);   
else
    [vtemp,repP] = lseval_sol(elem,getnode(S),ud,P(repP2(:)),ddl,getlevelsets(S));
end
v(:,:,repP2(repP)) = vtemp ;
    con = con + length(repP);
end
