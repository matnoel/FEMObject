function plotMeanLagrangeMultiplier(interfaces,lambda,varargin)
% function plotMeanLagrangeMultiplier(interfaces,lambda,varargin)
% Display the mean (mathematical expectation) of Lagrange multiplier lambda
...
elseif isa(interfaces,'Interface')
    ...
    if ischarin('displ',varargin)
        i = getcharin('displ',varargin);
        figure('Name',['Mean of lambda_' num2str(i) ' over interface #' num2str(interface.number)])
        % set(gcf,'Name',['Mean of lambda_' num2str(i) ' over interface #' num2str(interface.number)])
    else
        figure('Name',['Mean of lambda over interface #' num2str(interface.number)])
        % set(gcf,'Name',['Mean of lambda over interface #' num2str(interface.number)])
    end
    clf
    plot_sol(interface.S,mean(lambda{interface.number})','EdgeColor','interp',varargin{:});
    colormap(p.Results.colormap)
