function plotVarianceLagrangeMultiplierReference(interfaces,lambda_ref,varargin)
% function plotVarianceLagrangeMultiplierReference(interfaces,lambda_ref,varargin)
% Display the variance of reference Lagrange multiplier lambda_ref
...
elseif isa(interfaces,'Interface')
    ...
    if ischarin('displ',varargin)
        ...
        % set(gcf,'Name',['Variance of lambda_ref_' num2str(i) ' over interface #' num2str(interface.number)])
    else
        figure('Name',['Variance of lambda_ref over interface #' num2str(interface.number)])
        % set(gcf,'Name',['Variance of lambda_ref over interface #' num2str(interface.number)])
    end
    clf
    plot_sol(interface.S,variance(lambda_ref{interface.number})','EdgeColor','interp',varargin{:});
    colormap(p.Results.colormap)
