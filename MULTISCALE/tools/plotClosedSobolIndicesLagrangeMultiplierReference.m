function plotClosedSobolIndicesLagrangeMultiplierReference(interfaces,lambda_ref,alpha,varargin)
% function plotClosedSobolIndicesLagrangeMultiplierReference(interfaces,lambda_ref,alpha,varargin)
% Display the Closed Sobol indices of reference Lagrange multiplier lambda_ref 
...
if isa(interfaces,'Interfaces')
    ...
    if ischarin('displ',varargin)
        ...
        figure('Name',['Closed Sobol index of lambda_ref_' num2str(i) ' over interfaces #' num2str([numbers{:}]) ' for random variables #' num2str(alpha)])
        % set(gcf,'Name',['Closed Sobol index of lambda_ref_' num2str(i) ' over interfaces #' num2str([numbers{:}]) ' for random variables #' num2str(alpha)])
    else
        figure('Name',['Closed Sobol index of lambda_ref over interfaces #' num2str([numbers{:}]) ' for random variables #' num2str(alpha)])
        % set(gcf,'Name',['Closed Sobol index of lambda_ref over interfaces #' num2str([numbers{:}]) ' for random variables #' num2str(alpha)])
    end
    clf
    n = numel(interfaces);
    for k=1:n
        ...
        plot_sol(interface.S,SensitivityAnalysis.closedSobolIndices(lambda_ref{interface.number},alpha,d)','EdgeColor','interp',varargin{:});
