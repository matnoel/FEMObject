function plotTotalSobolIndicesAllSolutions(glob,patches,interfaces,U,w,lambda,alpha,varargin)
% function plotTotalSobolIndicesAllSolutions(glob,patches,interfaces,U,w,lambda,alpha,varargin)
% Display Total Sobol indices of multiscale solution u=(U,w),
...
if ischarin('displ',varargin)
    ...
    figure('Name',['Total Sobol index of u_' num2str(i) '=(U_' num2str(i) ',w_' num2str(i) '), U_' num2str(i) ', w_' num2str(i) ', lambda_' num2str(i) ' for random variables #' num2str(alpha)])
    % set(gcf,'Name',['Total Sobol index of u_' num2str(i) '=(U_' num2str(i) ',w_' num2str(i) '), U_' num2str(i) ', w_' num2str(i) ', lambda_' num2str(i) ' for random variables #' num2str(alpha)])
else
    figure('Name',['Total Sobol index of u=(U,w), U, w, lambda for random variables #' num2str(alpha)])
    % set(gcf,'Name',['Total Sobol index of u=(U,w), U, w, lambda for random variables #' num2str(alpha)])
end
clf
...
subplot(1+n,2,1)
dU = ndims(U);
sU_out = SensitivityAnalysis.totalSobolIndices(U*P_out',alpha,dU)';
...
for k=1:n
    ...
    plot_sol(interface.S,SensitivityAnalysis.totalSobolIndices(lambda{interface.number},alpha,dlambda)','EdgeColor','interp',varargin{:});
    colormap(p.Results.colormap)
    if p.Results.colorbar
        colorbar
    end
