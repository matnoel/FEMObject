function [norms,fieldtype,fieldstorage,fieldddl] = calc_error(group,node,M,u,fun,varargin)
% function [norms,fieldtype,fieldstorage,fieldddl] = calc_error(group,node,M,u,fun,varargin)
...
for j=1:getnbelem(group)
    ...
    ng = getcharin('intordersto',varargin,getorder(getPC(u))+2);
    ...
    xg = transfer(RANDVARS(stomesh{1}),RANDVARS(PC),gausssto.coord);
    u1 = randomeval(u,xg,M);
    ...
    for e =1:gausssto.nbgauss
        ...
        if isempty(subgaussin.w)
            ...
            [norms_elem{1}(e) norms_elem{2}(e)] = suberrorelem(elem,xnode,subgaussin,ue,fun,detJ,varargin{:});
        end
    end
