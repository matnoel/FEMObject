function elem = TRI3(node,numelem,connec,varargin)
% function elem = TRI3(node,numelem,connec,'material',mat,'option',option)
...
if nargin==0
    ...
    elem = struct();
    elem = class(elem,'TRI3',elemp,eleme);
elseif nargin==1 && isa(node,'NODE') && numel(node)==3
    elem = TRI3(node,1,1:3);
elseif nargin==1 && isa(node,'double') && size(node,1)==3
    elem = TRI3(NODE(node),1,1:3);
else
