function [u,err,x,PC_seq,err_seq,x_seq] = decompmatrix_leastsquares_iter(apc,N,fun,n,regul,param,cv,k,opts,varargin)
% function [u,err,x,PC_seq,err_seq,x_seq] = decompmatrix_leastsquares_iter(PC,N,fun,n,regul,param,cv,k,opts,varargin)
% Decomposition of a function of random variables on polynomial chaos basis
% by (regularized) least-squares minimization (or regression) using cross-validation technique
...
% 
% function [u,err,x,PC_seq,err_seq,x_seq] = decompmatrix_leastsquares_iter(PC,N,fun)
% ordinary least-squares (OLS) minimization (without regularization)
% min_{ u } ||y-A*u||_2^2
% 
% function [u,err,x,PC_seq,err_seq,x_seq] = decompmatrix_leastsquares_iter(PC,N,fun,'l0',param)
...
% 
% function [u,err,x,PC_seq,err_seq,x_seq] = decompmatrix_leastsquares_iter(PC,N,fun,'l1',param)
...
if nargin<9 || ~isfield(opts,'sampling')
    opts.sampling = 'fixed';
end
if nargin<9 || ~isfield(opts,'addsample')
    opts.addsample = 0.1;
end
if nargin<9 || ~isfield(opts,'maxsample')
    opts.maxsample = Inf;
end
if nargin<9 || ~isfield(opts,'basis')
    opts.basis = 'fixed';
end
if nargin<9 || ~isfield(opts,'maxcoeff')
    opts.maxcoeff = Inf;
end
if nargin<9 || ~isfield(opts,'algorithm')
    opts.algorithm = 'RMS';
end
if nargin<9 || ~isfield(opts,'bulkparam')
    opts.bulkparam = 0.5;
end
if nargin<9 || ~isfield(opts,'tol')
    opts.tol = 1e-4;
end
if nargin<9 || ~isfield(opts,'tolstagn')
    opts.tolstagn = 5e-2;
end
if nargin<9 || ~isfield(opts,'toloverfit')
    opts.toloverfit = 1.1;
end
if nargin<9 || ~isfield(opts,'correction')
...
parfor i=1:n
    if isa(y{i},'cell')
        for l=1:length(y{i})
            if opts.correction
                [u{i},err{i}] = solve_leastsquares(A,y{i}{l}',regul,param,cv,k,'correction');
            else
                [u{i},err{i}] = solve_leastsquares(A,y{i}{l}',regul,param,cv,k);
            end
            u{i}{l} = u{i}{l}';
            if display_iter
                fprintf(['| ' fP ' | ' fN ' | ' ferr ' |\n'],P,N,norm(err{i}{l}));
        ...
        if opts.correction
            [u{i},err{i}] = solve_leastsquares(A,y{i}',regul,param,cv,k,'correction');
        else
            [u{i},err{i}] = solve_leastsquares(A,y{i}',regul,param,cv,k);
        end
        u{i} = u{i}';
        if display_iter
            fprintf(['| ' fP ' | ' fN ' | ' ferr ' |\n'],P,N,norm(err{i}));
...
if strcmp(sampling,'adaptive')
    ...
    while ~convergence(err,opts.tol) && ~stagnation(err,err_old,opts.tolstagn)
        ...
        x = [x;x_add];
        x_seq{end+1} = x;
        y_add = calc_response_iter(fun,x_add,n,varargin{:});
        err_old = err;
        parfor i=1:n
            if isa(y{i},'cell')
                for l=1:length(y{i})
                    ...
                    if opts.correction
                        [u{i}{l},err{i}{l}] = solve_leastsquares(A,y{i}{l}',regul,param,cv,k,'correction');
                    else
                        [u{i}{l},err{i}{l}] = solve_leastsquares(A,y{i}{l}',regul,param,cv,k);
                    end
                    u{i}{l} = u{i}{l}';
                    PC_seq{i}{l}{end+1} = PC;
                    ...
                    if isa(y{i},'cell')
                        for l=1:length(y{i})
                            if update_basis(PC{i}{l},err{i}{l},err_old{i}{l},regul,N,opts)
                                if opts.correction
                                    funsolve = @(A) solve_leastsquares(A,y{i}{l}',regul,param,cv,k,'correction');
                                else
                                    funsolve = @(A) solve_leastsquares(A,y{i}{l}',regul,param,cv,k);
                                end
                                PC{i}{l} = adapt(PC{i}{l},funsolve,xpc,opts.algorithm,opts.bulkparam);
                                A{i}{l} = polyval(PC{i}{l},xpc);
                                err_old{i}{l} = err{i}{l};
                                if opts.correction
                                    [u{i}{l},err{i}{l}] = solve_leastsquares(A{i}{l},y{i}{l}',regul,param,cv,k,'correction');
                                else
                                    [u{i}{l},err{i}{l}] = solve_leastsquares(A{i}{l},y{i}{l}',regul,param,cv,k);
                                end
                                u{i}{l} = u{i}{l}';
                        ...
                        if update_basis(PC{i},err{i},err_old{i},regul,N,opts)
                            if opts.correction
                                funsolve = @(A) solve_leastsquares(A,y{i}',regul,param,cv,k,'correction');
                            else
                                funsolve = @(A) solve_leastsquares(A,y{i}',regul,param,cv,k);
                            end
                            PC{i} = adapt(PC{i},funsolve,xpc,opts.algorithm,opts.bulkparam);
                            A{i} = polyval(PC{i},xpc);
                            err_old{i} = err{i};
                            if opts.correction
                                [u{i},err{i}] = solve_leastsquares(A{i},y{i}',regul,param,cv,k,'correction');
                            else
                                [u{i},err{i}] = solve_leastsquares(A{i},y{i}',regul,param,cv,k);
                            end
                            u{i} = u{i}';
                            PC_seq{i}{end+1} = PC{i};
                            err_seq{i}{end+1} = norm(err{i});
