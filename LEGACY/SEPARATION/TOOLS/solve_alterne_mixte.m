...
% function [u,result] = solve_alterne_mixte(A,b,solver,varargin)
% Solution of a 2x2 block system Au=b
% A:2x2 cell containing operators in SEPMATRIX format
% b:2x1 cell containing vectors in SEPMATRIX format
% solver : SEPSOLVER
...
if param.residual
    ...
    bsym = cell(2,1);
    for i=1:2
        bsym{i} =  A{1,i}'*b{1} + A{2,i}'*b{2};
        for j=1:2
            Asym{i,j} =  A{1,i}'*  A{1,j} +   A{2,i}'*  A{2,j};
        end
    end
    A=Asym;
    b=bsym;
