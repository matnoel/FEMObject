function [u,result] = solve_alterne_bis(A,b,solver,varargin)
% function [u,result] = solve_alterne_bis(A,b,solver,varargin)
...
for i=1:param.maxorder
    ...
    ATUtilde = AT*Utilde;
    UAU = Utilde'*AU;
    UtildeATUtilde = Utilde'*ATUtilde;
    Ubu = Utilde'*bu;
            ...
            for j=1:dim %%%% BOUCLE DIMENSION j
                if param.adjoint == 1
                    if ismember(j,param.adjointdim)
                        ...
                        S = reduce(Ubtilde,j);
                        UtildeATUtilde.F(:,j) = AT.F(:,j);
                        M = reduce(UtildeATUtilde,j);
                        Utilde.F{j} = (M')\S;
                    else
                        Utilde.F{j} = U.F{j};
                    end
                ...
                for ii=1:A.m
                    ATUtilde.F{ii,j} = AT.F{ii,j}*Utilde.F{j};
                    UAU.F{ii,j} = Utilde.F{j}'*AU.F{ii,j};
                    UtildeATUtilde.F{ii,j} = Utilde.F{j}'*ATUtilde.F{ii,j};
                end
                Ubu.F(:,j) = bu.F(:,j);
                for ii=1:bu.m
                    Ubu.F{ii,j} = Utilde.F{j}'*Ubu.F{ii,j};
