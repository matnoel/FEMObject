...
% X : PCRADIALMATRIX de taille n-by-1  (vecteur aleatoires)
...
% varargin : arguments passees � la fonction ksdensity
...
L = 0;
Xcs = full(double(random(Xc,N))); % simulation de Xc pour evaluer sa densite de proba

for i=1:numel(Xc)
pXc = ksdensity(Xcs(i,:),Xs(i,:),varargin{:});
L = L + sum(log(pXc+eps));
end
%L = L / N ; 
return
