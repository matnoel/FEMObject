...
for k=0:restart %%% RESTARTS DE ARNOLDI
    ...
    for i=1:paramradial.nbfoncmaxsimul %%% CONSTRUCTION DE LA BASE DE KRYLOV
        f0=expectmtimes(bu,l0tilde);
        L0 = expectmtimes(A,l0,l0tilde);
        U = L0\f0;
        
        H0=sqrt(prodscal(U,U,Am));
        ...
        if paramradial.adjoint
            f0=expectmtimes(l0,l0)*Am*U;
            L0 = expectmtimes(A,l0,l0tilde)';
            Utilde = L0\f0;
            Utilde = Utilde/sqrt(prodscal(Utilde,Utilde,Am));
        else
            Utilde=U;
        end
        ...
        if paramradial.adjoint
            fU = (U'*Am*U)*l;
            aU = Utilde'*A*U;
            [ltilde,flag] = localstosolver(aU,fU);
        else
            ltilde =l;
        end
        l0 = l ; U0 = U ;
        ...
        switch errorindicator
            case 'reference'
                ...
                if update
                    utemp = PCRADIALMATRIX(V,[n,1],ltemp);
                else
